...
kvminit()
{
  // printf(":init global kernel pgtbl\n");
...
uvm2kvmcopy(pagetable_t pagetable, pagetable_t kpagetable, uint64 oldsz, uint64 newsz) {
  ...
  for (a = oldsz; a < newsz; a += PGSIZE) {
    ...
    *kpte = PA2PTE((uint64)PTE2PA(*pte)) | flags;
    ...
    // if (mappages(kpagetable, a, PGSIZE, (uint64)PTE2PA(*pte), PTE_FLAGS(*pte) & (~PTE_U)) != 0) {
    //   goto err;
    // }
  }
  return 0;
// err:
//   uvmunmap(kpagetable, PGROUNDUP(oldsz), (a - PGROUNDUP(oldsz)) / PGSIZE, 0);
//   return -1;
}